  ...
  if (done) {
    ...
    return (
      <div className="text-center space-y-5 py-4">
        ...
        <div>
          <div className="text-4xl font-black text-slate-900">{totalCorrect}/{totalTests}</div>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="bg-space-800 rounded-2xl p-5 border border-slate-200">
          ...
          <motion.div key={trainIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-space-900 rounded-xl p-5 border border-slate-300 text-center mb-4">
            <div className="text-5xl mb-2">{item.emoji}</div>
            <div className="text-slate-900 font-black text-lg">{item.name}</div>
            ...
            <div className="flex justify-center gap-2 mt-3">
              {item.features.map((f) => (
                <span key={f} className="bg-space-800 rounded-lg px-2 py-1 text-xs text-slate-400 font-bold border border-slate-200">{f}</span>
              ))}
            </div>
          </motion.div>

          <div className="text-xs text-slate-400 text-center mb-3">Training image {trainIdx + 1}/{theme.trainingItems.length}</div>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="bg-space-800 rounded-2xl p-5 border border-slate-200">
          ...
          <div className="bg-space-900 rounded-xl p-5 border border-slate-300 text-center mb-4">
            <div className="text-5xl mb-2">{item.emoji}</div>
            <div className="text-slate-900 font-black text-lg">{item.name}</div>
          </div>
          ...
          {picked === null ? (
            <div className="grid grid-cols-2 gap-3">
              {theme.categories.map((cat) => (
                <motion.button key={cat} onClick={() => classify(cat)} whileTap={{ scale: 0.95 }} className="py-5 rounded-xl border border-slate-200 bg-space-900 font-black text-slate-900 text-lg hover:border-pink-500/50 transition-colors">
                  {cat}
                </motion.button>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
              <div className={`p-4 rounded-xl text-sm font-bold ${picked === item.category ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}>
                ...
                {item.trickyNote && <div className="font-normal text-slate-600 mt-2">{item.trickyNote}</div>}
              </div>
              <button onClick={nextTest} className="w-full py-4 rounded-xl font-black text-slate-900 bg-pink-500 hover:bg-pink-400 btn-press transition-colors">
  ...
  return (
    <div className="space-y-5">
      <div className="bg-space-800 rounded-2xl p-5 border border-slate-200 text-center">
        ...
        <div className="text-3xl font-black text-slate-900 mb-2">{themeCorrect}/{theme.testItems.length}</div>
        ...
        <button onClick={nextTheme} className="w-full py-4 rounded-xl font-black text-slate-900 bg-pink-500 hover:bg-pink-400 btn-press transition-colors">
