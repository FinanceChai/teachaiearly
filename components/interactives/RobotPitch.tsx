    ...
    return (
      ...
      >
        <div className="text-center mb-3">
          ...
          <h2 className="text-xl font-bold text-slate-900">Your Robot Pitch</h2>
        </div>
        ...
        <div className="bg-space-900 rounded-xl p-5 border border-zinc-400/30 mb-5">
          ...
          <div className="bg-space-800 rounded-lg p-3 mb-3">
            <div className="text-xs text-slate-400 mb-1">THE PROBLEM</div>
            <div className="text-slate-900">{selProblem?.emoji} {selProblem?.label}</div>
          </div>

          <div className="bg-space-800 rounded-lg p-3 mb-3">
            <div className="text-xs text-slate-400 mb-2">AI POWERS</div>
          ...
          <div className="bg-space-800 rounded-lg p-3 mb-3">
            <div className="text-xs text-slate-400 mb-1">HOW IT LEARNS</div>
            <p className="text-slate-600 text-sm">{learningText}</p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-space-800 rounded-lg p-3">
              <div className="text-xs text-slate-400 mb-1">PRICE</div>
              <div className="text-slate-900 text-sm">{selPrice?.emoji} {selPrice?.label.split("—")[0]}</div>
            </div>
            <div className="bg-space-800 rounded-lg p-3">
              <div className="text-xs text-slate-400 mb-1">TARGET BUYER</div>
              <div className="text-slate-900 text-sm">{selBuyer?.emoji} {selBuyer?.label}</div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-space-800 rounded-2xl p-6 border border-slate-200">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-bold text-slate-900">🚀 Robot Pitch Builder</h2>
      ...
      <div className="flex gap-1 mb-5">
        {STEPS.map((s, i) => (
          <div key={i} className="flex-1 text-center">
            ...
            <span className={`text-xs ${i === step ? "text-zinc-400" : "text-slate-400"}`}>{s.label}</span>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          <motion.div key="s0" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}>
            <h3 className="text-slate-900 font-bold mb-3">Choose a Real-World Problem</h3>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {PROBLEMS.map((p) => (
                ...
                >
                  ...
                  <div className="text-slate-900 text-sm font-bold mt-1">{p.label}</div>
                </motion.button>
              ))}
            </div>
            <motion.button
        ...
        {step === 1 && (
          <motion.div key="s1" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}>
            <h3 className="text-slate-900 font-bold mb-3">Design Your Robot</h3>
            <input
              type="text"
              value={robotName}
              ...
              maxLength={25}
              className="w-full bg-space-900 border-2 border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold mb-3 placeholder:text-slate-400 focus:border-zinc-400 focus:outline-none"
            />
            <p className="text-slate-600 text-sm mb-2">Body type:</p>
              ...
              {ROBOT_TYPES.map((t) => (
                <motion.button
                  ...
                  className={`p-2 rounded-lg border text-center text-sm ${
                    robotType === t.id ? "border-zinc-400 bg-zinc-400/10 text-slate-900" : "border-slate-200 bg-space-900 text-slate-600"
                  }`}
                >
            ...
            <p className="text-slate-600 text-sm mb-2">Size:</p>
              ...
              {SIZES.map((s) => (
                <motion.button
                  ...
                  className={`p-2 rounded-lg border text-center text-xs ${
                    robotSize === s.id ? "border-zinc-400 bg-zinc-400/10 text-slate-900" : "border-slate-200 bg-space-900 text-slate-600"
                  }`}
                >
            ...
            <div className="flex gap-3">
              ...
              <motion.button
                ...
                className={`flex-1 py-3 font-bold rounded-xl ${STEPS[1].valid ? "bg-zinc-400 text-space-900" : "bg-slate-700 text-slate-400 cursor-not-allowed"}`}
              >
                Next →
              </motion.button>
            </div>
        ...
        {step === 2 && (
          <motion.div key="s2" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}>
            <h3 className="text-slate-900 font-bold mb-1">Pick 3 AI Features</h3>
            <p className="text-slate-600 text-sm mb-3">Selected: {features.length}/3</p>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {AI_FEATURES.map((f) => (
                ...
                >
                  ...
                  <div className="text-slate-900 font-bold text-xs">{f.label}</div>
                  <div className="text-slate-600 text-xs">{f.desc}</div>
                </motion.button>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <motion.button
                ...
                className={`flex-1 py-3 font-bold rounded-xl ${features.length === 3 ? "bg-zinc-400 text-space-900" : "bg-slate-700 text-slate-400 cursor-not-allowed"}`}
              >
                Next →
              </motion.button>
            </div>
        ...
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}>
            <h3 className="text-slate-900 font-bold mb-3">How Does {robotName} Learn &amp; Improve?</h3>
            ...
            <textarea
              ...
              maxLength={300}
              rows={4}
              className="w-full bg-space-900 border-2 border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-zinc-400 focus:outline-none resize-none mb-1"
            />
            <p className="text-slate-400 text-xs mb-4">{learningText.length}/300 characters (min 10)</p>
            <div className="flex gap-3">
              ...
              <motion.button
                ...
                className={`flex-1 py-3 font-bold rounded-xl ${STEPS[3].valid ? "bg-zinc-400 text-space-900" : "bg-slate-700 text-slate-400 cursor-not-allowed"}`}
              >
                Next →
              </motion.button>
            </div>
        ...
        {step === 4 && (
          <motion.div key="s4" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}>
            <h3 className="text-slate-900 font-bold mb-3">Business Plan</h3>
            <p className="text-slate-600 text-sm mb-2">Price point:</p>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {PRICE_POINTS.map((p) => (
                <motion.button
                  key={p.id}
                  ...
                  className={`p-2 rounded-lg border text-left text-sm ${
                    price === p.id ? "border-zinc-400 bg-zinc-400/10 text-slate-900" : "border-slate-200 bg-space-900 text-slate-600"
                  }`}
                >
            ...
            <p className="text-slate-600 text-sm mb-2">Who would buy this?</p>
            <div className="grid grid-cols-3 gap-2 mb-4">
              {BUYERS.map((b) => (
                <motion.button
                  key={b.id}
                  ...
                  className={`p-2 rounded-lg border text-center text-xs ${
                    buyer === b.id ? "border-zinc-400 bg-zinc-400/10 text-slate-900" : "border-slate-200 bg-space-900 text-slate-600"
                  }`}
                >
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(3)} className="py-3 px-4 text-slate-600 hover:text-sky-600">← Back</button>
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                onClick={() => STEPS[4].valid && setFinished(true)}
                disabled={!STEPS[4].valid}
                className={`flex-1 py-3 font-bold rounded-xl ${STEPS[4].valid ? "bg-zinc-400 text-space-900" : "bg-slate-700 text-slate-400 cursor-not-allowed"}`}
              >
                See My Pitch! 🚀
              </motion.button>
            </div>
