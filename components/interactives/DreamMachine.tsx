    ...
    return (
      <div className="space-y-5">
        ...
        <div className="bg-space-800 rounded-2xl p-5 border border-slate-200">
          ...
          <motion.div key={stepIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-space-900 rounded-xl p-5 border border-slate-300 text-center mb-4">
            <div className="text-5xl mb-3">{step.icon}</div>
            <div className="text-xs text-blue-400 font-black mb-1">STEP {stepIdx + 1}</div>
            <div className="text-xl font-black text-slate-900 mb-2">{step.name}</div>
    ...
    <div className="space-y-5">
      ...
      <div className="bg-space-800 rounded-2xl p-5 border border-slate-200">
        ...
        <div className="bg-space-900 rounded-xl p-4 border border-slate-300 mb-4">
          ...
          <div className="text-slate-900 font-black text-lg">&quot;{prompt.prompt}&quot;</div>
        </div>

        <div className="space-y-3 mb-4">
          {prompt.results.map((r, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} className="bg-space-900 rounded-xl p-4 border border-slate-300">
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-900 font-black text-sm">{r.label}</span>
        ...
        {!showLesson ? (
          <button onClick={() => setShowLesson(true)} className="w-full py-4 rounded-xl font-black text-slate-900 bg-blue-500 hover:bg-blue-400 btn-press transition-colors">
            Why Are They Different? &rarr;
          </button>
        ) : (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 text-sm text-slate-600">
