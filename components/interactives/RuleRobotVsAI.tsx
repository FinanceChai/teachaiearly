    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-space-800 rounded-2xl p-5 border border-red-500/30">
          <div className="text-center mb-4">
            ...
            <div className="font-black text-slate-900 text-sm">Rule Robot</div>
          ...
          <div className="space-y-2">
            {RULE_ITEMS.map((item) => (
              ...
              >
                ...
                <span className="text-slate-600 font-bold flex-1 text-left">{item.label}</span>
                {ruleResults[item.id] && (
                  <span
        ...
        {/* AI Robot */}
        <div className="bg-space-800 rounded-2xl p-5 border border-sky-200">
          <div className="text-center mb-4">
            <div className="text-3xl mb-2">🧠</div>
            <div className="font-black text-slate-900 text-sm">AI Robot</div>
          ...
          {aiPhase === "train" && (
            <>
              ...
              {allSorted && (
                <button
                  onClick={runTest}
                  className="w-full mt-3 bg-mint-400 hover:bg-mint-300 text-white font-black text-sm py-2 rounded-xl transition-colors"
                >
                  Test the AI →
                </button>
              )}
                ...
                {TEST_ITEMS.map((item, i) => (
                  <div key={item.id} className="bg-space-900 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xl">{item.emoji}</span>
                      <span className="text-sm text-slate-900 font-bold">{item.label}</span>
                    </div>
                    <div className="text-xs text-sky-500 font-bold">
                      AI says: {testResults[i]}
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-3 bg-mint-400/20 rounded-xl p-3 text-xs text-sky-400 font-bold text-center">
      ...
      {aiPhase === "test" && !completed && (
        <div className="bg-space-800 rounded-2xl p-5 border border-slate-200">
          <div className="font-black text-slate-900 mb-2">What did you learn?</div>
          ...
          <button
            onClick={handleDone}
            className="w-full mt-4 bg-mint-400 hover:bg-mint-300 text-white font-black py-3 rounded-xl transition-colors"
          >
            Got it! ✓
          </button>
        </div>
